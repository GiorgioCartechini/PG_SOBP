...
logicTreatmentRoom(0), physicalTreatmentRoom(0),
physiBeamLineSupport(0), physiBeamLineCover(0), physiBeamLineCover2(0),
physiMonitorLayer1(0), physiMonitorLayer2(0),
ScatteringFoil(0), logicScatteringFoil(0), 
physiScatteringFoil(0), ridgeFilterPhys(0), 
preCollimator(0), physiPreCollimator(0), 
Collimator(0), physiCollimator(0), 
solidAirTube(0), solidAirPreTube(0), 
physiAirTube(0), physiAirPreTube(0),
aRegion(0),
outerScattererRing(0), 
innerScattererRing(0), SupportFoil(0), physiOuterScattererRing(0), 
physiInnerScattererRing(0), physiSupportFoil(0), logicTumor(0), phyisicalTumor(0), defaultTumortoWPPosition(0)
{
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // -----------------------------
    ...
    const G4double worldX = 300.0 *cm;
    const G4double worldY = 300.0 *cm;
    ...
    G4Material* worldMaterial = airNist;
    ...
    HadrontherapyBeamSecondScatterer();
    WaterPhantom_and_Tumor();
...
void DetectorConstruction::SetDefaultDimensions()
{
    ...
    G4double defaultScatteringFoilXSize = 0.39*mm;
    ...
    G4double defaultScatteringFoilXPosition = -149.*cm;
	...
	// SUPPORT FOIL: it is a thin foil that provides the
    // final diffusion of the beam.
    G4double defaultSupportFoilXSize = 0.5*mm;
    SupportFoilXSize = defaultSupportFoilXSize;
    
    G4double defaultSupportFoilYSize = 100*mm;
    SupportFoilYSize = defaultSupportFoilYSize;
    
    G4double defaultSupportFoilZSize = 100 *mm;
    SupportFoilZSize = defaultSupportFoilZSize;
    
    //    G4double defaultSupportFoilXPosition = -148.*cm;
    //    G4double defaultSupportFoilXPosition = -70.*cm;
    G4double defaultSupportFoilXPosition = -98.95*cm;
    SupportFoilXPosition = defaultSupportFoilXPosition;
    
    G4double defaultSupportFoilYPosition =  0 *mm;
    SupportFoilYPosition = defaultSupportFoilYPosition;
    
    G4double defaultSupportFoilZPosition =  0 *mm;
    SupportFoilZPosition = defaultSupportFoilZPosition;
    
    //RIDEG FILTER
    ridgeXPosition = -90.*cm; //8cm from double ring
    ...
    G4Material* BrainNist =  G4NistManager::Instance()->FindOrBuildMaterial("G4_MYLAR");
    ...
    ScatteringFoilMaterial = leadNist;

    // Material of the scattering foil
    SupportFoilMaterial = aluminumNist;

    // Material of the second scattering foil
    outerScattererRingMaterial = aluminumNist; ;
    innerScattererRingMaterial = leadNist;
...
//    TUMOR
    defaultTumortoWPPosition = G4ThreeVector(0.,0.,0.)*cm;
    TumorR = 2.5*cm;
...
//    El31P
    ...
    //19F
        F19 = new G4Material("F19", 1.696*g/cm3, 1);
    ...
    PMMAF -> AddMaterial(PMMANist, (100-fFraction)*perCent);
    PMMAF -> AddMaterial(F19, (fFraction)*perCent);
...
////////////////////////////////////////////////////////////////////////
void DetectorConstruction::WaterPhantom_and_Tumor()
{
	// ------------------//
    // WATER PHANTOM	 //
    //-------------------//
    ...
    //Bone
    G4double WaterPhantom_Bone_sizeXYZ = 20.*cm; //cubic phantom
    G4Material* WaterPhantom_Bone_mat = G4NistManager::Instance()->FindOrBuildMaterial("G4_BONE_COMPACT_ICRU");

      G4Box* solidWaterPhantomBone = new G4Box("WaterPhantomBone",                       //its name
                                          0.5*WaterPhantom_Bone_sizeXYZ,
                                          0.5*WaterPhantom_Bone_sizeXYZ,
                                          0.5*WaterPhantom_Bone_sizeXYZ);     //its size

      G4LogicalVolume* logicWaterPhantomBone = new G4LogicalVolume(solidWaterPhantomBone,          //its solid
                                                          WaterPhantom_Bone_mat,           //its material
                                                          "WaterPhantomBoneLV");            //its name

      new G4PVPlacement(0,                     //no rotation
                        G4ThreeVector(0.5*WaterPhantom_Bone_sizeXYZ,0,0),       //la faccia in z del WP e' nell'origine
                        logicWaterPhantomBone,            //its logical volume
                        "WaterPhantomBone",               //its name
    ...
    G4double WaterPhantom_sizeXYZ = 19.5*cm; //cubic phantom
	G4Material* WaterPhantom_mat = G4NistManager::Instance()->FindOrBuildMaterial("G4_BRAIN_ICRP");
  
	G4Box* solidWaterPhantom = new G4Box("WaterPhantom",                       //its name
										0.5*WaterPhantom_sizeXYZ, 
										0.5*WaterPhantom_sizeXYZ, 
										0.5*WaterPhantom_sizeXYZ);     //its size
      
	G4LogicalVolume* logicWaterPhantom = new G4LogicalVolume(solidWaterPhantom,          //its solid
														WaterPhantom_mat,           //its material
    ...
    new G4PVPlacement(0,                     //no rotation
                      ...
                      logicWaterPhantomBone,                     //its mother  volume
                      false,                 //no boolean operation
                      0,                     //copy number
                      checkOverlaps);        //overlaps checking
   ...
   G4VSolid* solidTumor = new G4Orb("Tumor", TumorR);
    ...
    if (!aRegion)
    {
        aRegion = new G4Region("WaterPhantom");
        //logicWaterPhantom -> SetRegion(aRegion);
        aRegion->AddRootLogicalVolume( logicWaterPhantomBone);
...
/////////////////////////////////////////////////////////////////////////////
void DetectorConstruction::HadrontherapyBeamLineSupport()
{
    // ------------------//
    // BEAM LINE SUPPORT //
    //-------------------//
...
/////////////////////////////////////////////////////////////////////////////
void DetectorConstruction::HadrontherapyBeamMonitoring()
{
  // ----------------------------
  //   MONITOR CHAMBER
  // ----------------------------
...
void DetectorConstruction::HadrontherapyBeamScatteringFoils()
{
...
////////////////////////////////////////////////////////////////////////
void DetectorConstruction::HadrontherapyBeamSecondScatterer()
{
  // ---------------------------- //
  // THE SECOND SCATTERING SYSTEM //
  // ---------------------------- //
  // It is a dual-ring double-scattering system, composed by an outer ring of aluminium and
  // an inner ring made of lead
  // --> Uncomment following lines according to the double-ring setup needed (SMALL or BIG)

  /* Geometry for SMALL double-ring setup */
  
  //Half values!!!!(except for radius)
  const G4double outerScattererRingRadiusMin = 11.0*mm;
  const G4double outerScattererRingRadiusMax = 10.*cm;
  const G4double outerScattererRingHalfZSize = 2.5*mm;
  const G4double outerScattererRingHalfXPosition = -99.25*cm;
  const G4double innerScattererRingRadiusMin = 0.;
  const G4double innerScattererRingRadiusMax = 11.*mm;
  const G4double innerScattererRingHalfZSize = 0.75*mm;
  const G4double innerScattererRingHalfXPosition = -99.075*cm;


  /* Geometry for BIG double-ring setup */
  
  // const G4double outerScattererRingRadiusMin = 22.5*mm;
  // const G4double outerScattererRingRadiusMax = 10.*cm;
  // const G4double outerScattererRingHalfZSize = 8.*mm;
  // const G4double outerScattererRingHalfXPosition = -99.8*cm;
  // const G4double innerScattererRingRadiusMin = 0.;
  // const G4double innerScattererRingRadiusMax = 22.5*mm;
  // const G4double innerScattererRingHalfZSize = 2.75*mm;
  // //const G4double innerScattererRingHalfXPosition = -148.11*cm;
  // //  const G4double innerScattererRingHalfXPosition = -70.11*cm;
  // const G4double innerScattererRingHalfXPosition = -99.275*cm;
  ...
  G4RotationMatrix rt;
  rt.rotateY(mi);
  G4ThreeVector position = G4ThreeVector(outerScattererRingHalfXPosition, 0., 0.);
  G4Transform3D transform (rt, position);
  
  outerScattererRing = new G4Tubs("OuterScattererRing",
				  outerScattererRingRadiusMin,
				  outerScattererRingRadiusMax,
				  outerScattererRingHalfZSize,
				  0.*deg,
				  360.*deg);

  G4LogicalVolume* logicOuterScattererRing = new G4LogicalVolume(outerScattererRing,
								 outerScattererRingMaterial,
								 "OuterScattererRing");
  


  physiOuterScattererRing = new G4PVPlacement(G4Transform3D(rt,
							    G4ThreeVector(outerScattererRingHalfXPosition, 0., 0.)),
					      "OuterScattererRing",
					      logicOuterScattererRing,
					      physicalTreatmentRoom,
					      false,
					      0,
						  checkOverlaps);
  
  logicOuterScattererRing -> SetVisAttributes(blue);
  

  innerScattererRing = new G4Tubs("InnerScattererRing",
				  innerScattererRingRadiusMin,
				  innerScattererRingRadiusMax,
				  innerScattererRingHalfZSize,
				  0.*deg,
				  360.*deg);

  
  
  G4LogicalVolume* logicInnerScattererRing = new G4LogicalVolume(innerScattererRing,
								 innerScattererRingMaterial,
								 "InnerScattererRing");
  
  physiInnerScattererRing = new G4PVPlacement(G4Transform3D(rt, G4ThreeVector(innerScattererRingHalfXPosition,
										0.,
									      0.)),
					      "InnerScattererRing",
					      logicInnerScattererRing,
					      physicalTreatmentRoom,
					      false,
					      0,
					      checkOverlaps);
  
  logicInnerScattererRing -> SetVisAttributes(red);
  
  
  SupportFoil = new G4Box("SupportFoil",
			     SupportFoilXSize,
			     SupportFoilYSize,
			     SupportFoilZSize);
  
 
  G4LogicalVolume* logicSupportFoil = new G4LogicalVolume(SupportFoil,
					    SupportFoilMaterial,
					    "SupportFoil");
  
  physiSupportFoil = new G4PVPlacement(0, G4ThreeVector(SupportFoilXPosition,
							   SupportFoilYPosition,
							   SupportFoilZPosition),
					  "SeconSupportFoil",
                                              logicSupportFoil,
					  physicalTreatmentRoom,
					  false,
...
void DetectorConstruction::HadrontherapyRidgeFilter()
{
  // ---------------------------- //
  //         THE Ridge Filter    //
  // -----------------------------//
    ...
    for (int i = 0; i < numberOfLayers; i++)
      {
    	for (int j = 0; j < numberOfLayers; j++)
    	  {
	    singleTrapPositions.push_back({-0.08*cm+ridgeXPosition,
    		  minY + 2*i*sum_space,
    		  minZ + 2*j*sum_space,});
    	  }
      }
...
void DetectorConstruction::HadrontherapyBeamCollimators()
{
...
void DetectorConstruction::SetTumorToWaterPhantomPosition(G4ThreeVector Tumorpos)
{
	phyisicalTumor -> SetTranslation(defaultTumortoWPPosition+Tumorpos);
    G4RunManager::GetRunManager() -> GeometryHasBeenModified();
    G4cout  <<" The Tumor collimator is translated to" << Tumorpos/mm <<"mm along the (X,Y,Z) axis" << G4endl;
}
...
void DetectorConstruction::SetScatteringFoilXSize(G4double value)
{
...
void DetectorConstruction::SetPreCollimatorXSize(G4double value)
{
...
void DetectorConstruction::SetPreCollimatorXPosition(G4double value)
{
...
void DetectorConstruction::SetAirTubeYSize(G4double value)
{
...
void DetectorConstruction::SetAirTubeZSize(G4double value)
{
...
/////////////////////////////////////////////////////////////////////////////
void DetectorConstruction::SetScattererMaterial(G4String materialChoice)
{
  if (G4Material* pttoMaterial = G4NistManager::Instance()->FindOrBuildMaterial(materialChoice, false) )
    {
      if (pttoMaterial)
